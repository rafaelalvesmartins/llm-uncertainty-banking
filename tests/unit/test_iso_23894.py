...
from lub.compliance.frameworks import iso_23894
...
def test_regime_is_iso_23894() -> None:
    assert iso_23894.REGIME is Regime.ISO_23894
...
def test_crosswalk_key_matches_regime_name() -> None:
    ...
    regime-backed frameworks; ``"ISO_23894"`` is the enum member name.
    """
    assert iso_23894.CROSSWALK_KEY == "ISO_23894"
    assert iso_23894.CROSSWALK_KEY == Regime.ISO_23894.name


def test_title_is_human_readable_iso_string() -> None:
    assert iso_23894.TITLE == "ISO/IEC 23894:2023 — AI risk management"
    assert isinstance(iso_23894.TITLE, str)


def test_dunder_all_is_exact() -> None:
    assert iso_23894.__all__ == ["CROSSWALK_KEY", "REGIME", "TITLE", "get_controls"]
...
def test_get_controls_is_non_empty(controls: list[dict]) -> None:
    """The 2026-04-22 verification in crosswalk.py records 4 ISO_23894 refs."""
...
def test_get_controls_returns_defensive_copy() -> None:
    ...
    """
    first = iso_23894.get_controls()
    second = iso_23894.get_controls()
...
def test_get_controls_matches_crosswalk_helper(controls: list[dict]) -> None:
    ...
    assert controls == list(get_all_controls_for_regime(Regime.ISO_23894))
...
def test_module_satisfies_compliance_framework_protocol() -> None:
    ...
    assert isinstance(iso_23894, ComplianceFrameworkProtocol)
